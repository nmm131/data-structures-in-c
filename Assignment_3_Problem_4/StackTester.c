...
int main(int argc, const char * argv[]) {
    ...
    // Show the allocation count when we setup the temporary stack
    PrintAllocations ("Before temporary stack");
    Stack STemp = initStack();
    PrintAllocations ("After temporary stack is created");
    ...
    while (!empty(S))
    {
        ...
        //create data to fill in with real values
        UserData D2;

        //call a peek
        PrintStackItem ("peek called, data is", D2 = peek(S));
        /*
        //call a pop and push it into the temporary stack
        if (empty(STemp) || D2.taskNumber < peek(STemp).taskNumber) {
                push(STemp, pop(S));
        } else {
        ...
        */
        push(STemp, pop(S));
        //print the temporary stack item
        printf("pop  called, data is %d %s and was added to temporary stack", D2.taskNumber, D2.taskName);
        //append AllocationCount to the end of the last print
        PrintAllocations("");
        //change counter value to show increased number of pops by 1
        numOfPops++;
    }
