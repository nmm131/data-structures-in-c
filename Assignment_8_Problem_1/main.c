...
typedef struct {
    ...
    unsigned int allowedSpeed;      // speed data for the edge in miles per hour from the data file
    unsigned int isToll;      // toll data for the edge in is/is-not a toll from the data file
    } MapDatabase;
// Code to perform a Dijkstra analysis for the Simple Garmin
#include "SimpleGarmin.h"
// test files supported with the code
#define testfile "California.in"
...
MapDatabase ReadEdgeData (FILE * in)
{
    ...
    MapDatabase MDB;
    // fill in the data
    fscanf (in, "%d%d%d", &MDB.distance, &MDB.allowedSpeed, &MDB.isToll);
...
unsigned int BestTime (MapDatabase WD)
{
    unsigned int time = (60.0 * WD.distance) / WD.allowedSpeed;
    return time;
}

unsigned int AvoidTolls (MapDatabase WD)
{
    if (WD.isToll == 0) {
        return WD.distance;
    } else {
...
int main()
{
    ...
    MySimpleGarmin(G, "Middletown", MinimumDistance, "(mileage, using minimum distance)");
    MySimpleGarmin(G, "Waterbury", MinimumDistance, "(mileage, using minimum distance)");
    MySimpleGarmin(G, "Middletown", BestTime, "(time, using minimum time)");
    MySimpleGarmin(G, "Waterbury", BestTime, "(time, using minimum time)");
    MySimpleGarmin(G, "Middletown", AvoidTolls, "(tolls, avoiding tolls)");
    MySimpleGarmin(G, "Waterbury", AvoidTolls, "(tolls, avoiding tolls)");
    return 0;
} // end main
